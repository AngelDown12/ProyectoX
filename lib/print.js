...
console.log(
  ...
  ┃ ❖ ${chalk.white.bold(mid.idioma_code === 'es' ? 'Usuario:' : 'User:')} ${chalk.white('%s')} / ${chalk.bgMagentaBright.bold(user.role.replace(/\*/g, ''))}
  ...
  m.chat.includes('@s.whatsapp.net') ? ${chalk.white.bold(mid.idioma_code === 'es' ? 'Privado:' : 'Private')} ${chalk.greenBright(${m.sender} ~${user.name || conn.getName(m.sender)})} : 
  m.chat.includes('@g.us') ? ${chalk.white.bold(mid.idioma_code === 'es' ? 'Grupo:' : 'Group:')} ${chalk.magentaBright(${m.chat} ~${chat})} : 
  m.chat.includes('@newsletter') ? ${chalk.white.bold(mid.idioma_code === 'es' ? 'Canal:' : 'Channel:')} ${chalk.yellowBright(${m.chat} ~${chat})} : '',
...
async function formatMessageStubType(messageStubType, name_user) {
  switch (messageStubType) {
    ...
    case 1: return 'Revocado'
    case 2: return 'Texto cifrado'
    case 20: return 'Se ha creado un grupo'
    case 21: return 'Nombre del grupo modificado'
    case 22: return 'Se cambió la imagen del grupo'
    case 23: return 'Nuevo enlace del grupo'
    case 24: return 'Nueva descripción del grupo'
    case 25: return 'Restricciones del grupo cambiada'
    case 26: return 'Se configuró quien puede enviar mensajes en el grupo'
    case 27: return ${name_user} se ha unido al grupo
    case 28: return ${name_user} ha sido eliminado del grupo
    case 29: return ${name_user} es nuevo admin. del grupo
    case 30: return ${name_user} dejó de ser admin. del grupo
    case 31: return Se ha invitado a ${name_user} al grupo
    case 32: return ${name_user} ha salido del grupo
    case 145: return Se configuró "aprobar miembros nuevo" en el grupo
    case 171: return Se configuró "agregar a otros miembros" en el grupo
